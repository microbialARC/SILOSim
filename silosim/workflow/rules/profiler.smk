rule profiler:
    ...
    params:
        ...
        min_mge_len = config["min_mge_len"],
    ...
    threads:
        config["threads"]
    output:
        bin_summary = os.path.join(config["output"], "profiler",f"{genome_name}_bin_summary.csv"),
        chr_bins_rds = os.path.join(config["output"], "profiler",f"{genome_name}_chr_bins.RDS"),
        concat_fasta = os.path.join(config["output"], "profiler",f"{genome_name}_concat.fasta"),
        entropy_csv = os.path.join(config["output"], "profiler",f"{genome_name}_entropy.csv"),
        entropy_rds = os.path.join(config["output"], "profiler",f"{genome_name}_entropy.RDS"),
        mges_seq_fasta = os.path.join(config["output"], "profiler",f"{genome_name}_mges_seq.fasta"),
        mges_csv = os.path.join(config["output"], "profiler",f"{genome_name}_mges.csv"),
        mges_rds = os.path.join(config["output"], "profiler",f"{genome_name}_mges.RDS"),
        new_pos_rds = os.path.join(config["output"], "profiler",f"{genome_name}_new_pos.RDS"),
        plot_df_rds = os.path.join(config["output"], "profiler",f"{genome_name}_plot_df.RDS"),
        position_coverage_rds = os.path.join(config["output"], "profiler",f"{genome_name}_position_coverage.RDS"),
        profiler_plot_pdf = os.path.join(config["output"], "profiler",f"{genome_name}_profiler_plot.pdf"),
        snps_sum_rds = os.path.join(config["output"], "profiler",f"{genome_name}_snps_sum.RDS")
    script:
        # The profiler.R script is a command-line R script that accepts arguments.
        ...
        os.path.join(BASE_PATH,"scripts","profiler.R")